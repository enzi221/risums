...
local function main(output)
  ...
  local allBlocks = prelude.extractNodes('lb-stage', output)
  local body = nil
  if #allBlocks == 1 then
    body = allBlocks[1]
  end
  ...
  if not data or not data.objective or not data.phase or not data.episodes then
    ...
    return '<lb-lazy identifier="lb-stage"></lb-lazy>'
  end
  ...
  setChatVar(triggerId, 'lb-stage-key', triggerId)
  setChatVar(triggerId, 'lb-stage-raw', body.content)
  setChatVar(triggerId, 'lb-stage-objective', json.encode(data.objective))
  setChatVar(triggerId, 'lb-stage-phase', json.encode(data.phase))
  setChatVar(triggerId, 'lb-stage-episodes', json.encode(deepEncodedEpisodes))
  setChatVar(triggerId, 'lb-stage-divergence', data.divergence)
  setChatVar(triggerId, 'lb-stage-comment', data.comment)

  return '<lb-stage key="' .. triggerId .. '">' .. xor(body.content) .. '</lb-stage>'
end
